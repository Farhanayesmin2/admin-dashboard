...
const DashboardSidebar = ({ sidebarOpen, setSidebarOpen }) => {
	...
	return (
		<aside
			ref={sidebar}
			className={`absolute mb-12 pb-20  left-0 top-0 z-9999 flex h-screen w-72.5 flex-col overflow-y-hidden  duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0 ${
		...
		>
			...
			<div className="flex text-black bg-purple-500 items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5">
				...
				>
					<FiChevronRight className="fill-current" size={20} />
				</button>
			</div>
			{/* <!-- SIDEBAR HEADER --> */}
			...
			<div className=" text-black bg-purple-500 flex flex-col overflow-y-none  duration-300 ease-linear">
				{/* <!-- Sidebar Menu --> */}
				<nav className="mt-5 no-scrollbar py-4 pl-4 lg:mt-9 lg:pl-6">
					{/* <!-- Menu Group --> */}
					<div>
										...
										<>
											...
											>
												<RxDashboard
													...
													viewBox="0 0 20 20"
												/>
												<span className="pr-12">Dashboard </span>
												<BsChevronDown
								...
								{(handleClick, open) => {
									return (
										<>
											...
											>
												<AiOutlineForm className="" size={20} />
												Forms
												<BsChevronDown
													className={` absolute right-0 top-1/2 -translate-y-1/2 fill-current ${
														open && "rotate-180"
								...
								{(handleClick, open) => {
									return (
										<React.Fragment>
											...
											{/* <!-- Dropdown Menu Start --> */}
											<div
												className={`translate  text-black transform overflow-hidden ${
													!open && "hidden"
												}`}
											>
												<ul className="mt-4 mb-5.5  text-black flex flex-col gap-2.5 pl-6">
													<li>
														<NavLink
															to="/auth/signin"
															className={({ isActive }) =>
																"group  text-black relative flex items-center gap-2.5 rounded-md px-4 font-medium  duration-300 ease-in-out hover:text-black " +
																(isActive && "text-black")
															}
														>
															Sign In
